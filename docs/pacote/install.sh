...
if [ "$LANG_SEL" = en ]; then cat <<'TXT'
  ...
  1. Copy  {UP}.up  to the ROOT of the card, renamed to:
  ...
  5. Check under  Settings -> Information :   {TELA}
...
TXT
  ...
  1. Copie  {UP}.up  para a RAIZ do cartao, renomeando para:
  ...
  5. Confira em  Configurar -> Informacoes :   {TELA}
...
TXT
    ...
    [ -f "$HERE/cabo/flash_{UP}.sh" ] || \
        die "$(t 'cabo/flash_{UP}.sh is missing.' 'falta o cabo/flash_{UP}.sh.')"
    OPENPOD_LANG="$LANG_SEL" export OPENPOD_LANG
    sh "$HERE/cabo/flash_{UP}.sh" "$(dirname "$TOOL")"
}

# ------------------------------------------------------------------ main
[ -f "$UP" ] || { echo "{UP}.up not found / nao encontrado"; exit 1; }
