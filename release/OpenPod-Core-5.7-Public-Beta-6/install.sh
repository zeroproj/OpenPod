#!/bin/sh
# OpenPod 5.7 Beta 6 — installer / instalador
set -eu
export LC_ALL=C

HERE=$(cd "$(dirname "$0")" && pwd)
UP="$HERE/OpenPod_Beta6.up"
DEV=301a:2801
...
if [ "$LANG_SEL" = en ]; then cat <<'TXT'
  ...
  1. Copy  OpenPod_Beta6.up  to the ROOT of the card, renamed to:
  ...
  5. Check under  Settings -> Information :   OpenPod 5.7 Beta 6
...
TXT
  ...
  1. Copie  OpenPod_Beta6.up  para a RAIZ do cartao, renomeando para:
  ...
  5. Confira em  Configurar -> Informacoes :   OpenPod 5.7 Beta 6
...
TXT
    ...
    [ -f "$HERE/cabo/flash_OpenPod_Beta6.sh" ] || \
        die "$(t 'cabo/flash_OpenPod_Beta6.sh is missing.' 'falta o cabo/flash_OpenPod_Beta6.sh.')"
    OPENPOD_LANG="$LANG_SEL" export OPENPOD_LANG
    sh "$HERE/cabo/flash_OpenPod_Beta6.sh" "$(dirname "$TOOL")"
}

# ------------------------------------------------------------------ main
[ -f "$UP" ] || { echo "OpenPod_Beta6.up not found / nao encontrado"; exit 1; }

hd "OpenPod - 5.7 Public Beta 6"
pick_lang
