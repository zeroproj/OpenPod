#!/bin/sh
# OpenPod Reload Alpha 0.1 — installer / instalador
set -eu
export LC_ALL=C

HERE=$(cd "$(dirname "$0")" && pwd)
UP="$HERE/OpenPod_Reload_Alpha_0.1.up"
DEV=301a:2801
...
if [ "$LANG_SEL" = en ]; then cat <<'TXT'
  ...
  1. Copy  OpenPod_Reload_Alpha_0.1.up  to the ROOT of the card, renamed to:
  ...
  5. Check under  Settings -> Information :   OpenPod Reload Alpha 0.1
...
TXT
  ...
  1. Copie  OpenPod_Reload_Alpha_0.1.up  para a RAIZ do cartao, renomeando para:
  ...
  5. Confira em  Configurar -> Informacoes :   OpenPod Reload Alpha 0.1
...
TXT
    ...
    [ -f "$HERE/cabo/flash_OpenPod_Reload_Alpha_0.1.sh" ] || \
        die "$(t 'cabo/flash_OpenPod_Reload_Alpha_0.1.sh is missing.' 'falta o cabo/flash_OpenPod_Reload_Alpha_0.1.sh.')"
    OPENPOD_LANG="$LANG_SEL" export OPENPOD_LANG
    sh "$HERE/cabo/flash_OpenPod_Reload_Alpha_0.1.sh" "$(dirname "$TOOL")"
}

# ------------------------------------------------------------------ main
[ -f "$UP" ] || { echo "OpenPod_Reload_Alpha_0.1.up not found / nao encontrado"; exit 1; }

hd "OpenPod - Reload Alpha 0.1"
pick_lang
