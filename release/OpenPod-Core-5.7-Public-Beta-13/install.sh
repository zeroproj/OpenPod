#!/bin/sh
# OpenPod 5.7 B13 — installer / instalador
set -eu
export LC_ALL=C

HERE=$(cd "$(dirname "$0")" && pwd)
UP="$HERE/OpenPod_Beta13.up"
DEV=301a:2801
...
if [ "$LANG_SEL" = en ]; then cat <<'TXT'
  ...
  1. Copy  OpenPod_Beta13.up  to the ROOT of the card, renamed to:
  ...
  5. Check under  Settings -> Information :   OpenPod 5.7 B13
...
TXT
  ...
  1. Copie  OpenPod_Beta13.up  para a RAIZ do cartao, renomeando para:
  ...
  5. Confira em  Configurar -> Informacoes :   OpenPod 5.7 B13
...
TXT
    ...
    [ -f "$HERE/cabo/flash_OpenPod_Beta13.sh" ] || \
        die "$(t 'cabo/flash_OpenPod_Beta13.sh is missing.' 'falta o cabo/flash_OpenPod_Beta13.sh.')"
    OPENPOD_LANG="$LANG_SEL" export OPENPOD_LANG
    sh "$HERE/cabo/flash_OpenPod_Beta13.sh" "$(dirname "$TOOL")"
}

# ------------------------------------------------------------------ main
[ -f "$UP" ] || { echo "OpenPod_Beta13.up not found / nao encontrado"; exit 1; }

hd "OpenPod - 5.7 Public Beta 13"
pick_lang
