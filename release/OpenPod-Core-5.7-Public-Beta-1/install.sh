#!/bin/sh
# OpenPod Beta 1 — installer / instalador
set -eu
export LC_ALL=C

HERE=$(cd "$(dirname "$0")" && pwd)
UP="$HERE/OpenPod_Beta1.up"
DEV=301a:2801
...
if [ "$LANG_SEL" = en ]; then cat <<'TXT'
  ...
  5. Check under  Settings -> Information :   OpenPod Beta 1
...
TXT
  ...
  5. Confira em  Configurar -> Informacoes :   OpenPod Beta 1
...
# ------------------------------------------------------------------ main
[ -f "$UP" ] || { echo "OpenPod_Beta1.up not found / nao encontrado"; exit 1; }
