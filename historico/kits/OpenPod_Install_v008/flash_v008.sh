#!/bin/sh
# flash_v008.sh — OpenPod. GERADO por tools/make_install_kit.py.
# NAO EDITE A MAO: as constantes sao calculadas a partir das imagens.
#
# fundo da tela de abertura passa a preto, para o logo se fundir
#
...
#
# USO   sudo sh flash_v008.sh [/opt/smartlink_flash]
...
DEV=301a:2801
WORK=$(pwd)/openpod_flash_v008
LOG="$WORK/flash_v008.log"
WROTE=0
...
die() {
    ...
    if [ "$WROTE" -ne 0 ]; then
        ...
        log "***   sudo $TOOL --id $DEV write_flash 0x122000 0 0x1000 orig_122000.bin"
...
log "======================================================================"
log " OpenPod — gravacao do V008   (2 setores, 8 KiB)"
log " 0x00D000 +0x1000   <- v008_D000.bin"
log " 0x122000 +0x1000   <- v008_122000.bin"
...
ck_file v008_D000.bin bdb36efaef38fcd748c8dd0f85f0376a9097700133c87a4189a4a4b1a19c6b28
ck_file v008_122000.bin 23d44d20cd914ab860eedd25f938eece99af4a059e8e229b3a33e6195fef06ac
ck_file orig_D000.bin c3d16a15b7df6a8feac96169d93da82a5562ae3d17a94bf8906c2d24fc507091
ck_file orig_122000.bin dc3b6a4a0150763564474047400cebac7b4e331820901505eacf3644083c7c48
...
ck "ptable" 53248 64 55990cc24fb4c7a073ce2d5f3cd66cccf4a9486528f0ccb8d21977e97aca3a4a
ck "FIRM" 57344 1647984 6fd51dab368d63bc10262a46a6b50d4c72bf6bf847d7d675c656aa7b510f80a2
...
log "   muda   : fundo da tela de abertura passa a preto, para o logo se fundir"
...
wr "4/6 1/2" 0xD000 v008_D000.bin bdb36efaef38fcd748c8dd0f85f0376a9097700133c87a4189a4a4b1a19c6b28
wr "4/6 2/2" 0x122000 v008_122000.bin 23d44d20cd914ab860eedd25f938eece99af4a059e8e229b3a33e6195fef06ac
rm -f "$WORK/sec.bin"
...
log "======================================================================"
if [ "$F" -eq 0 ]; then
    log " [6/6] RESULTADO: V008 GRAVADO E VERIFICADO"
    log ""
...
log "======================================================================"
log ""
log "  leve de volta: before.bin  after.bin  flash_v008.log"
log ""
