#!/bin/sh
# flash_v030.sh — OpenPod. GERADO por tools/make_install_kit.py.
# NAO EDITE A MAO: as constantes sao calculadas a partir das imagens.
#
# Recuperacao por cartao SD: Configurar > Configuracao de fabrica arma o flag 6 no PMU
#
...
#
# USO   sudo sh flash_v030.sh [/opt/smartlink_flash]
...
DEV=301a:2801
WORK=$(pwd)/openpod_flash_v030
LOG="$WORK/flash_v030.log"
WROTE=0
...
die() {
    ...
    if [ "$WROTE" -ne 0 ]; then
        ...
        log "***   sudo $TOOL --id $DEV write_flash 0x37000 0 0x1000 base_37000.bin"
...
log "======================================================================"
log " OpenPod — gravacao do V030   (3 setores, 12 KiB)"
log " 0x1A3000 +0x1000   <- v030_1A3000.bin"
log " 0x037000 +0x1000   <- v030_37000.bin"
log " 0x109000 +0x1000   <- v030_109000.bin"
...
ck_file v030_1A3000.bin 62857a18018e93a3401b2227511a9387b6b2e3a23bcd9150fef7ae3f1d81f9ac
ck_file v030_37000.bin 18132b95bbfc6142c482f2382454fe1433cc560f11f20a5ef254c1271e165377
ck_file v030_109000.bin ce2cce9a87161453a8da363218527885fa4c7f301b6df8c9f0b8605e857409d7
ck_file base_1A3000.bin 6f4ef0b382a9fa4d2792e8446613308544b251e2d512ea1b5d3b83ce8f1d10e4
ck_file base_37000.bin ad7facb2586fc6e966c004d7d1d16b024f5805ff7cb47c7a85dabd8b48892ca7
...
log "   muda   : Recuperacao por cartao SD: Configurar > Configuracao de fabrica arma o flag 6 no PMU"
...
wr "4/6 1/3" 0x1A3000 v030_1A3000.bin 62857a18018e93a3401b2227511a9387b6b2e3a23bcd9150fef7ae3f1d81f9ac
wr "4/6 2/3" 0x37000 v030_37000.bin 18132b95bbfc6142c482f2382454fe1433cc560f11f20a5ef254c1271e165377
wr "4/6 3/3" 0x109000 v030_109000.bin ce2cce9a87161453a8da363218527885fa4c7f301b6df8c9f0b8605e857409d7
rm -f "$WORK/sec.bin"
...
ck2 "FIRM" 57344 1647984 9d6de3625d1e5806a1df640387b9f01cc13ca8db41575528e7a7b76eef276f1d
ck2 "TONE" 1708032 8248 7f2882af95534ec56c6261ac14c74ddcf9a83c8deb4b5e97e333b2ee3f8b4ace

log ""
log "======================================================================"
if [ "$F" -eq 0 ]; then
    log " [6/6] RESULTADO: V030 GRAVADO E VERIFICADO"
    log ""
...
log "======================================================================"
log ""
log "  leve de volta: before.bin  after.bin  flash_v030.log"
log ""
