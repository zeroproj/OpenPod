#!/bin/sh
# flash_v006.sh — OpenPod. GERADO por tools/make_install_kit.py.
# NAO EDITE A MAO: as constantes sao calculadas a partir das imagens.
#
# tema: cor de texto passa de branco para cinza em todas as telas que usam o tema
#
...
#
# USO   sudo sh flash_v006.sh [/opt/smartlink_flash]
...
DEV=301a:2801
WORK=$(pwd)/openpod_flash_v006
LOG="$WORK/flash_v006.log"
WROTE=0
...
log "======================================================================"
log " OpenPod — gravacao do V006   (2 setores, 8 KiB)"
log " 0x00D000 +0x1000   <- v006_D000.bin"
log " 0x121000 +0x1000   <- v006_121000.bin"
...
ck_file v006_D000.bin 7effe32c8e51fe0dbb9e6a8a69d31a3c1e9c06cd29ce90a901788e3050649dfe
ck_file v006_121000.bin f02709501538c03fff136ae91cabe6f7897c31538973ef9e1d0a0406f1f935d7
...
ck "ptable" 53248 64 5f1e69c4190fb60471b6a7597920fdc8e74a68a4c2b2fcec457cab26aca62bb4
ck "FIRM" 57344 1647984 ffc5e6736fe58f86063dfad23b6c36f67e51820693010a1c1af53b9eb0848318
...
log "   muda   : tema: cor de texto passa de branco para cinza em todas as telas que usam o tema"
...
wr "4/6 1/2" 0xD000 v006_D000.bin 7effe32c8e51fe0dbb9e6a8a69d31a3c1e9c06cd29ce90a901788e3050649dfe
wr "4/6 2/2" 0x121000 v006_121000.bin f02709501538c03fff136ae91cabe6f7897c31538973ef9e1d0a0406f1f935d7
rm -f "$WORK/sec.bin"
...
ck2 "ptable" 53248 64 b1329cf4330fd862ad2fd743dbfe59305841fbcdba97b3e454f699e089e5ad60
ck2 "FIRM" 57344 1647984 4d6c709d2b7c3e00e04612c6c430f5f989e340eaab0f7ba5542807f287ec5dab
ck2 "TONE" 1708032 8248 7f2882af95534ec56c6261ac14c74ddcf9a83c8deb4b5e97e333b2ee3f8b4ace

log ""
log "======================================================================"
if [ "$F" -eq 0 ]; then
    log " [6/6] RESULTADO: V006 GRAVADO E VERIFICADO"
    log ""
...
log "======================================================================"
log ""
log "  leve de volta: before.bin  after.bin  flash_v006.log"
log ""
