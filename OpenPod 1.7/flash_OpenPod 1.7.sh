#!/bin/sh
# flash_OpenPod 1.7.sh — OpenPod. GERADO por tools/make_install_kit.py.
...
#
# USO   sudo sh flash_OpenPod 1.7.sh [/opt/smartlink_flash]
...
DEV=301a:2801
WORK=$(pwd)/openpod_flash_OpenPod 1.7
LOG="$WORK/flash_OpenPod 1.7.log"
WROTE=0
...
die() {
    ...
    if [ "$WROTE" -ne 0 ]; then
        ...
        log "***   sudo $TOOL --id $DEV write_flash 0x147000 0 0x1000 base_147000.bin"
...
log "======================================================================"
log " OpenPod — gravacao do OPENPOD 1.7   (5 setores, 20 KiB)"
log " 0x1A3000 +0x1000   <- OpenPod 1.7_1A3000.bin"
log " 0x1A5000 +0x1000   <- OpenPod 1.7_1A5000.bin"
log " 0x10A000 +0x1000   <- OpenPod 1.7_10A000.bin"
log " 0x123000 +0x1000   <- OpenPod 1.7_123000.bin"
log " 0x147000 +0x1000   <- OpenPod 1.7_147000.bin"
...
ck_file OpenPod 1.7_1A3000.bin e4c75816c039dc648d8344f622b4ff8d6a0cbfdd5d6a215037fab0f3bed82837
ck_file OpenPod 1.7_1A5000.bin d9e49226a07c887194a031d043634b486b9625eff696782511ef0a15df14b60f
ck_file OpenPod 1.7_10A000.bin 86bdec86749947b3f4d7ed01831068bd1558996a17ff008d6411c206b837181b
ck_file OpenPod 1.7_123000.bin 56f6996c47eceda5076a0202b0b0b678c5135fede20b97a29a025e798d856723
ck_file OpenPod 1.7_147000.bin 453df7b12745c56408119e5da039f5ee5a7665ab26df95aae8a77f6b3d15c305
...
ck_file base_147000.bin 685f1aa127b3997c4052a52b904f2c9c927dc717c1d75caaa34c474dc4657887
...
log "   muda   : Titulo na barra + selecao quadrada + M volta / VOL desce"
...
wr "4/6 1/5" 0x1A3000 OpenPod 1.7_1A3000.bin e4c75816c039dc648d8344f622b4ff8d6a0cbfdd5d6a215037fab0f3bed82837
wr "4/6 2/5" 0x1A5000 OpenPod 1.7_1A5000.bin d9e49226a07c887194a031d043634b486b9625eff696782511ef0a15df14b60f
wr "4/6 3/5" 0x10A000 OpenPod 1.7_10A000.bin 86bdec86749947b3f4d7ed01831068bd1558996a17ff008d6411c206b837181b
wr "4/6 4/5" 0x123000 OpenPod 1.7_123000.bin 56f6996c47eceda5076a0202b0b0b678c5135fede20b97a29a025e798d856723
wr "4/6 5/5" 0x147000 OpenPod 1.7_147000.bin 453df7b12745c56408119e5da039f5ee5a7665ab26df95aae8a77f6b3d15c305
rm -f "$WORK/sec.bin"
...
ck2 "FIRM" 57344 1647984 02b3741a906cb1b590d2540a5c5e07751ef1df7fd1c56b0dadfd6cc791664cc1
ck2 "TONE" 1708032 8248 7f2882af95534ec56c6261ac14c74ddcf9a83c8deb4b5e97e333b2ee3f8b4ace

log ""
log "======================================================================"
if [ "$F" -eq 0 ]; then
    log " [6/6] RESULTADO: OPENPOD 1.7 GRAVADO E VERIFICADO"
    log ""
...
log "======================================================================"
log ""
log "  leve de volta: before.bin  after.bin  flash_OpenPod 1.7.log"
log ""
